Hi,
This is NOT a real moodle plugin (but it conveniently reuses the db connetion $DB !). This is only here to keep track of the link checker code that runs against the hub's table.
...
Note: perhaps this could become a plugins in future where automatic link checking/fingerprinting can happen from everywhere. This is so that with a ton of sites - this can be scaleable in future.
