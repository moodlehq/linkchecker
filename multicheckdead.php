...
$GLOBALS['lockfile'] = "/opt/linkcheck/moodle19/sites/.multicheckdead.lock";
declare(ticks = 1);
...
$GLOBALS['sitebufferlimit'] = 50;
$GLOBALS['maximumunreachable'] = 2;
$GLOBALS['timelinkchecked'] = time()-86400;
$GLOBALS['tablename'] = "registry";
$GLOBALS['siteselectorsql'] = "SELECT `id`,`unreachable`,`sitename`,`url`,`public`, `timeunreachable`, `score`, `errormsg`, `moodlerelease`, `serverstring` FROM `".$CFG->prefix.$tablename."` WHERE `unreachable`>=%d AND `override`=0 AND `timelinkchecked`<=%d AND `id`<%d ORDER BY `id` DESC LIMIT %d";
$GLOBALS['sitessofar'] = null;
$GLOBALS['totalsites'] = count_records_select($tablename, "`unreachable`>=$maximumunreachable AND `override`=0 AND `timelinkchecked`<=$timelinkchecked");
$GLOBALS['maxcurltimeout'] = 120;
$GLOBALS['maxconnectiontimeout'] = 50;
...
function update_site(&$site, $score='', $unreachable=0, $errormessage='', $moodlerelease=null, $serverstring=null) {
    ...
    if (strpos($errormessage, "Connection time-out after") !== false || strpos($errormessage, "SSL connection timeout") !== false) {
      $unreachable = 0;
      $score = 0;
    ...
    update_record($tablename, $updatedsite);
    return true;
}
...
function fill_site_buffer() {
    global $sitebuffer, $sitebufferlimit, $CFG, $siteselectorsql, $sitessofar, $totalsites, $maximumunreachable, $timelinkchecked;
...
function writeline($id, $url, $outcome='F', $score='0', $redirects='0', $errorno='', $errormsg='', $moodlerelease='') {
    ...
    static $count;
    if ($header==null) {
        echo "\nC   |ID        | URL                                               | P/F | Score |Redir |ErNum| Version                 | Error Msg";
        echo "\n------------------------------------------------------------------------------------------------------------------------------------------------------";
        $header = true;
    }
    if ($count==null) {
    ...
    $score = (strlen($score)<6)?str_pad($score,6):substr($score,0,6);
